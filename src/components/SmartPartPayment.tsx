...
const SmartPartPayment: React.FC = () => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        try {
          ...
          const legalFeeIndex = feeHeaders.findIndex(h => typeof h === 'string' && h.includes('LEGAL FEE'));
          const processingFeeIndex = feeHeaders.findIndex(h => typeof h === 'string' && h.includes('PROCESSING FEE'));
          const bcIndex = feeHeaders.findIndex(h => typeof h === 'string' && h.includes('BC'));
          const bankIndex = feeHeaders.findIndex(h => typeof h === 'string' && h.includes('BANK'));
  ...
  return (
    <Card className="w-full max-w-6xl mx-auto p-4">
      <CardHeader>
        <CardTitle className="text-2xl font-bold text-center">Smart Part Payment</CardTitle>
            ...
            <div className="border p-4 rounded-lg bg-gray-50">
              ...
              {!isValidLoanNoFormat(data[currentIndex].loanNo) && (
                <div className="p-3 mb-4 text-sm text-yellow-800 rounded-lg bg-yellow-50" role="alert">
                  <span className="font-medium">Warning!</span> The loan number "{data[currentIndex].loanNo}" might be incorrect. It should be in a format like "PKD/3489".
                </div>
              )}
              <pre className="bg-white p-2 rounded overflow-auto">
                {JSON.stringify({ ...data[currentIndex], officeId: getOfficeId(data[currentIndex].loanNo) }, null, 2)}
              </pre>
            </div>
